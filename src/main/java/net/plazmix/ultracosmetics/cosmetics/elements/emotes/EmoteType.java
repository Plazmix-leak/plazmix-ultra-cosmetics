...
import net.plazmix.ultracosmetics.util.TexturedSkullFactory;
...
public class EmoteType extends CosmeticMatType<Emote> {
    ...
    public static EmoteType valueOf(String s) {
        for (EmoteType emoteType : VALUES) {
            if (emoteType.getSignature().equalsIgnoreCase(s)) return emoteType;
        }
        return null;
    }
    ...
    public static final EmoteType CRY = new EmoteType("EMOTE_CRY", 1, true);
    public static final EmoteType ANGRY = new EmoteType("EMOTE_ANGRY", 1, true);
    public static final EmoteType HAPPY = new EmoteType("EMOTE_HAPPY", 1,true);
    public static final EmoteType CHEEKY = new EmoteType("EMOTE_CHEEKY",1, true);
    public static final EmoteType LOVE = new EmoteType("EMOTE_LOVE", 2, true);
    public static final EmoteType DEAL_WITH_IT = new EmoteType("EMOTE_DEAL_WITH_IT", 3, true);
    public static final EmoteType COOL = new EmoteType("EMOTE_COOL", 2, true);
    public static final EmoteType SURPRISED = new EmoteType("EMOTE_SURPRISED", 1, true);
    public static final EmoteType WINK = new EmoteType("EMOTE_WINK" ,2, true);


    //TODO: Перенести этот пиздец в разные классы
    ...
    public EmoteType(String signature, int ticksPerFrame, Boolean isEnable) {
        super(Category.EMOTES, signature, UCMaterial.PLAYER_HEAD, Emote.class, ServerVersion.v1_8_R1, isEnable);
    ...
    @Override
    public Emote equip(CosmeticPlayer cosmeticPlayer, UltraCosmetics ultraCosmetics) {
        if (cosmeticPlayer.getBukkitPlayer().getInventory().getHelmet() != null) {
            if (cosmeticPlayer.getCurrentHat() != null) {
                cosmeticPlayer.removeHat();
            } else if (cosmeticPlayer.getCurrentEmote() != null) {
                cosmeticPlayer.removeEmote();
            } else {
                cosmeticPlayer.getBukkitPlayer().sendMessage("&c&lОшибка, Вы должны снять шлем, чтобы экипировать эмоцию!");
                return null;
            }
        }
        Emote cosmetic = null;
        try {
            cosmetic = getClazz().getDeclaredConstructor(CosmeticPlayer.class, EmoteType.class, UltraCosmetics.class).newInstance(cosmeticPlayer, this, ultraCosmetics);
